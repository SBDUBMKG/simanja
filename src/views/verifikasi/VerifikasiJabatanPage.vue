                    ...
                    <DataTable v-if="jabatanLoaded" class="table table-sm table-hover table-bordered table-responsive-xl display">
                        <thead>
                            <tr class="table-head">
                                ...
                                <th width="25%" class="column-title">FUNGSIONAL</th>
                                <th width="25%" class="column-title">SATKER</th>
                                <th width="10%" class="column-title">TAHUN ANJAB</th>
                                <th width="15%" class="column-title">STATUS</th>
                            ...
                            <tr v-for="(jabatan, index) in daftarJabatan" :key="jabatan.id_jabatan">
                                ...
                                <td>
                                    <div class="d-flex justify-content-around">
                                        <button @click="editBtn(jabatan.id_jabatan)" class="btn btn-warning btn-sm ml-1 mr-1">
    ...
    methods: {
        ...
        async loadDataJabatan() {
            try {
                ...
                const response = await axios.get(`${process.env.VUE_APP_BACKENDHOST}/jabatan/verificator`, config);
        ...
        editBtn (id_jabatan) {
            this.$router.push({ name: 'VerifikasiBiodata', params: { jabatanid: id_jabatan } })    
        },
